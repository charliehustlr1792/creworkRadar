...
export function StatsBar({ total, highIntent, lastRefreshed }: StatsBarProps) {
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-px mb-5 sm:mb-6 rounded-xl overflow-hidden border border-gray-200 shadow-sm">
      {stats.map((s, i) => (
        <div
          key={i}
          className={`relative bg-white px-4 sm:px-5 py-4 sm:py-5 ${s.glow ? 'bg-emerald-50/50' : ''}`}
        >
          <div className="text-[9px] text-gray-400 tracking-widest font-mono mb-1.5 sm:mb-2">{s.label}</div>
          ...
          {s.glow && (
            <div className="absolute top-3 right-3 w-1.5 h-1.5 rounded-full bg-emerald-400 shadow-[0_0_6px_2px_rgba(52,211,153,0.4)]" />
          )}
        </div>
      ))}
    </div>
